...
public class BaseController : Base, ITurnParticipant
{
    // 공격 전략
    ...
    protected GameManagerEx MyState;

    public bool isAlive => IsAlive();
    public string Name => gameObject.name;

    // 외부 접근 허용
    public GameManagerEx State
    {
        get => MyState;
        private set
        {
    ...
    public override bool Init()
    {
        ...
        MyState = new GameManagerEx(new CharactorData());
   ...
   public void SetSkeletonAsset(string path)
    ...
    {
        WeaponData weaponData = Managers.s_WeaponLoader.GetWeaponData(_weaponType);
        if (weaponData == null)
        {
            Console.WriteLine("Failed Load WeaponData : BaseController()");
            return; 
        }
...
#region 데미지
    ...
    public virtual IEnumerator AttacktoMove(GameObject _target)
    {
        ...
        while (Vector3.Distance(transform.position, finalTargetPosition) > stopDistance)
        {
            ...
            float duration = MyState.AttackSpeed * 0.03f; 
    ...
    {
        transform.position = Managers.TransformManager.MoveToTarget(transform.position, RunAreaPosition, MyState.AttackSpeed);
    }
